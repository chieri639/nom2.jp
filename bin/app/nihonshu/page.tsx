...
export default async function NihonshuIndexPage() {
    const { contents: sakes } = await getSakes({ limit: 100 });
    ...
    return (
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '60px 24px', fontFamily: 'system-ui, sans-serif' }}>
                    ...
                    <Link href={`/nihonshu/${sake.id}`} key={sake.id} style={{ textDecoration: 'none', color: 'inherit' }}>
                        <div style={{ 
                            ...
                            <div style={{ height: '200px', backgroundColor: '#f3f4f6', position: 'relative' }}>
                                {sake.imageUrl ? (
                                    <img src={sake.imageUrl} alt={sake.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                ) : (
                            ...
                            <div style={{ padding: '1.5rem', flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
                                <p style={{ fontSize: '0.875rem', color: '#6b7280', marginBottom: '0.5rem', fontWeight: 700 }}>{sake.brewery}</p>
                                <h2 style={{ fontSize: '1.25rem', fontWeight: 800, color: '#1f2937', marginBottom: '0.75rem', lineHeight: 1.4 }}>{sake.name}</h2>
                                <p style={{ fontSize: '1.125rem', color: '#bfa758', fontWeight: 700, marginBottom: '1rem' }}>
                                    {sake.price ? `¥${sake.price} (参考)` : '価格未定'}
                                </p>
            ...
            {sakes.length === 0 && (
                <div style={{ textAlign: 'center', padding: '4rem', color: '#6b7280' }}>日本酒データがまだありません。</div>
            )}
        </div>
    );
}
