        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '60px 24px', fontFamily: 'system-ui, sans-serif' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '2rem' }}>
                {breweries.map((brewery) => (
                    <Link href={`/brewery/${brewery.id}`} key={brewery.id} style={{ textDecoration: 'none', color: 'inherit' }}>
                        <div style={{ 
                            ...
                            <div style={{ height: '200px', backgroundColor: '#f3f4f6', position: 'relative' }}>
                                {brewery.imageUrl ? (
                                    <img src={brewery.imageUrl} alt={brewery.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                ) : (
                            ...
                            <div style={{ padding: '1.5rem', flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
                                <span style={{ display: 'inline-block', alignSelf: 'flex-start', padding: '0.25rem 0.75rem', backgroundColor: '#fef3c7', color: '#92400e', borderRadius: '9999px', fontSize: '0.75rem', fontWeight: 'bold', marginBottom: '1rem' }}>
                                    酒蔵
                                </span>
                                <h2 style={{ fontSize: '1.25rem', fontWeight: 800, color: '#1f2937', marginBottom: '0.75rem', lineHeight: 1.4 }}>{brewery.name}</h2>
            ...
            {breweries.length === 0 && (
                <div style={{ textAlign: 'center', padding: '4rem', color: '#6b7280' }}>酒蔵データがまだありません。</div>
            )}
        </div>
    );
}
